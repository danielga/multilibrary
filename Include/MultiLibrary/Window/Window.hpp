...
enum WindowCallback
{
	WINDOW_CALLBACK_CLOSE,
	WINDOW_CALLBACK_REFRESH,
	WINDOW_CALLBACK_RESIZE,
	WINDOW_CALLBACK_REPOSITION,
	WINDOW_CALLBACK_FOCUS,
	WINDOW_CALLBACK_ICONIFY,
	WINDOW_CALLBACK_FRAMEBUFFER_RESIZE,
	WINDOW_CALLBACK_KEY,
	WINDOW_CALLBACK_CHARACTER,
	WINDOW_CALLBACK_MOUSE,
	WINDOW_CALLBACK_CURSOR_POSITION,
	WINDOW_CALLBACK_CURSOR_ENTER,
	WINDOW_CALLBACK_SCROLL,
};
...
enum CursorMode
{
	CURSOR_MODE_NORMAL = 212993,
	CURSOR_MODE_HIDDEN = 212994,
	CURSOR_MODE_DISABLED = 212995
};
...
public:
	Window( );
	Window( const VideoMode &mode, const std::string &title, const WindowSettings &window_settings = WindowSettings( ), const ContextSettings &context_settings = ContextSettings( ) );
	virtual ~Window( );

	bool IsValid( ) const;

	bool Create( const VideoMode &mode, const std::string &title, const WindowSettings &window_settings = WindowSettings( ), const ContextSettings &context_settings = ContextSettings( ) );
	void Close( );

	bool ShouldClose( ) const;
	void SetShouldClose( bool close = false );
	...
	void SetCallbackActive( WindowCallback type, bool active = true );
...
private:
	static void CloseCallback( WindowHandle window );
	static void RefreshCallback( WindowHandle window );
	static void ResizeCallback( WindowHandle window, int width, int height );
	static void RepositionCallback( WindowHandle window, int x, int y );
	static void FocusCallback( WindowHandle window, int focused );
	static void IconifyCallback( WindowHandle window, int iconified );
	static void FramebufferResizeCallback( WindowHandle window, int width, int height );
	static void KeyPressCallback( WindowHandle window, int key, int scancode, int action, int modifiers );
	static void CharacterInputCallback( WindowHandle window, unsigned int codepoint );
	static void MousePressCallback( WindowHandle window, int button, int action, int modifiers );
	static void CursorRepositionCallback( WindowHandle window, double xpos, double ypos );
	static void CursorEnterCallback( WindowHandle window, int entered );
	static void ScrollCallback( WindowHandle window, double xoffset, double yoffset );

	WindowHandle window_internal;
};
